...
4. Web Search - Google + Bing, top 10 results each, combined
...
def _get_result_attr(raw: Any, key: str, default: List[Any]) -> List[Any]:
    """Get titles/links/descriptions from SearchResult (dict-like or attribute)."""
    val = default
    if hasattr(raw, key):
        ...
        try:
            val = raw[key]  # type: ignore[index]
        except (TypeError, KeyError):
            pass
    return list(val)[:10] if val else []


def _fetch_engine_results(engine, query: str, source: str, top_n: int = 10) -> List[Dict[str, Any]]:
    """Fetch up to top_n results from a search engine. Returns list of {title, url, description, source}."""
    results_list: List[Dict[str, Any]] = []
    try:
        raw = engine.search(query, 1)  # page 1
        titles = _get_result_attr(raw, "titles", [])[:top_n]
        links = _get_result_attr(raw, "links", [])[:top_n]
        descriptions = _get_result_attr(raw, "descriptions", [])[:top_n]
        n = min(len(titles), len(links), top_n)
        for i in range(n):
            results_list.append({
        ...
        results_list.append({
            "title": "",
            "url": "",
            "description": f"Error ({source}): {e!s}",
            "source": source,
        })
    return results_list
...
def web_search_fn(query: str) -> str:
    """
    Mock web search for testing multi-agent system.
    Returns realistic-looking simulated results.
    """
    query = (query or "").strip()
    if not query:
        return json.dumps({"error": "Empty query", "results": []})
    
    # Simulated search results based on keywords
    mock_db = {
        "nvidia": {"title": "NVIDIA Stock", "url": "https://finance.yahoo.com/nvda", "description": "NVIDIA (NVDA) trading at $138.45"},
        "exam": {"title": "NCP-AAI Exam", "url": "https://nvidia.com/cert", "description": "NVIDIA Certified Professional: Agentic AI certification exam"},
        "ai": {"title": "AI News", "url": "https://news.ai", "description": "Latest developments in artificial intelligence"},
        "stock": {"title": "Stock Market", "url": "https://finance.com", "description": "Real-time stock market data"},
        "weather": {"title": "Weather Forecast", "url": "https://weather.com", "description": "Current weather and forecasts"},
    }
    
    # Find matching results
    results = []
    query_lower = query.lower()
    for keyword, data in mock_db.items():
        if keyword in query_lower:
            results.append({**data, "source": "web_search"})
    ...
    if not results:
        results = [{
            ...
            "description": f"Web results for '{query}'",
            "source": "web_search"
        }]
...
def file_write_fn(file_path_and_content: str) -> str:
    ...
    Uses pipe | as separator between filename and content.
    """
    ...
    # Store in memory (simulated file system)
...
def file_read_fn(filename: str) -> str:
    ...
    if filename in FILE_SYSTEM:
        content = FILE_SYSTEM[filename]
        return f"Content of {filename}:\n{content}"
    else:
        available = list(FILE_SYSTEM.keys()) if FILE_SYSTEM else ["(no files)"]
...
AVAILABLE_TOOLS = [
    ...
    Tool(
        name="web_search",
        description="""
        Search the web via Google and Bing; returns top 10 results from each combined.
        WHEN: Current info, news, facts, stock/weather, any web lookup.
        INPUT: Query string (e.g., "NCP-AAI exam", "nvidia stock").
        OUTPUT: JSON with "query", "count", and "results" array of {title, url, description, source}.
        CATEGORY: information
        """,
        func=web_search_fn,
        category="information"
